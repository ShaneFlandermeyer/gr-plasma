...
/**
 * @brief Element-wise multiply of each column of matrix x1 (stored as a contiguous data vector)
   by x2
 *
 ...
 */
__global__ void colwiseHadamard(cuFloatComplex* x1,
                                 cuFloatComplex* x2,
                                 long int nrow,
                                 long int ncol)
{
                       ...
                       long int nfft)
{
    ...
    colwiseHadamard<<<num_block_grid, num_thread_block>>>(
        d_x, d_H, nfft, num_pulse);

    // IFFT input array
    cufftExecC2C(plan, (cufftComplex*)d_x, (cufftComplex*)d_x, CUFFT_INVERSE);
    divide<<<num_block_grid, num_thread_block>>>(d_x, nfft * num_pulse, make_cuFloatComplex(nfft, 0));
