                                            ...
                                            int pdu_length)
{
    return gnuradio::make_block_sptr<pdu_file_source_impl>(
        data_filename, meta_filename, offset, length, pdu_length);
}
...
pdu_file_source_impl::pdu_file_source_impl(const std::string& data_filename,
    ...
    : gr::block("pdu_file_source",
      ...
      d_offset(offset),
      d_length(length),
      d_pdu_length(pdu_length)
{
    // Read I/Q data from file
    d_data = ::plasma::read<gr_complex>(d_data_filename, d_offset, d_length);
...
{
    if (d_pdu_length == -1) {
        pmt::pmt_t pdu_data = pmt::init_c32vector(d_data.size(), d_data.data());
        message_port_pub(pmt::mp("out"), pmt::cons(d_meta, pdu_data));
    } else {
        int n_pdu = d_data.size() / d_pdu_length;
        for (int i = 0; i < n_pdu; i++) {
            pmt::pmt_t pdu_data =
                pmt::init_c32vector(d_pdu_length, d_data.data() + i * d_pdu_length);
            message_port_pub(pmt::mp("out"), pmt::cons(d_meta, pdu_data));
            // Sleep for 10 ms
            // TODO: Remove this
